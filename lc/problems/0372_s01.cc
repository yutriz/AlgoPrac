...
/* 
 * 
 * a^(x1*10^y1 + x2*10^y2 + **** + xn)
 * a^(x1*10^y1) * a^(x2*10^y2) * ..* a^xn
 * a = 1337*x + c
 * mod(a) = c
 * and mod(a^(xm*10^ym))
 *    =mod(multi(a*..xm*10^ym times..a))
 *    =mod(multi(mod(a)..that times ..mod(a)))
 *
 * and mod(n1*n2*n3*...*nn)
 *    =mod(mod(n1)*mod(n2)*...*mod(nn))
 *
 * so  answer
 *    =mod(mod(a)*...b times ...mod(a))
 *
 */ 
int noMoreThan10times(int a, int times) {
    if (a==1) 
        return 1;
    int ret = 1;
    int moda = a % 1337;
...
int superPower(int a, vector<int>& b) {
    if (a==1 || b.empty())
        return 1;
    int b_end = b[b.size()-1];
    b.pop_back();
    int last_dig = noMoreThan10times(a, b_end);
    int before_last = noMoreThan10times(superPower(a,b), 10);
    return (last_dig*before_last) % 1337;
